...
const Appointments = () => {
  ...
  const [lastBookedAppointmentId, setLastBookedAppointmentId] =
    useState(null);
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.appointmentId) {
          setLastBookedAppointmentId(data.appointmentId);
        }
      } else {
  ...
  const payLastBooking = async () => {
    if (!lastBookedAppointmentId || !token) return;
    await startAppointmentPayment({
      ...
      onPaid: () => {
        setLastBookedAppointmentId(null);
    ...
    docInfo && (
      <div>
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
          ...
          {lastBookedAppointmentId && token && docInfo && (
            <div className="mt-4 p-4 rounded-xl border border-indigo-200 bg-indigo-50/80 max-w-lg">
              <p className="font-medium text-gray-800">Booking confirmed</p>
              <p className="text-sm text-gray-600 mt-1">
                Pay the consultation fee ({currencySymbol}
                {docInfo.fee}) online with Razorpay, or pay later from My
                Appointments.
              </p>
              <div className="flex flex-wrap gap-3 mt-4">
                <button
                  type="button"
                  onClick={payLastBooking}
                ...
                <button
                  ...
                  className="px-6 py-2 rounded-full border border-gray-400 text-sm text-gray-700"
                >
                  Pay later — My appointments
                </button>
              </div>
            </div>
          )}
