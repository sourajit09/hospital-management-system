...
// Fallback demo admin credentials so the panel remains accessible even if env vars
// are missing in the deployed environment (use env values when present).
...
const loginAdmin = async (req, res) => {
  try {
    ...
    if (valid || allowBypass) {
      // Always sign with the configured admin creds so middleware verification passes
      const token = jwt.sign(ADMIN_EMAIL + ADMIN_PASSWORD, JWT_SECRET);
      return res.json({
        success: true,
        message: valid
          ? "Admin Logged in Successfully"
          : "Admin bypass login enabled (using configured admin account).",
        token,
      });
    }
